...
# Check if the script has been passed the "-y" argument
while [[ $# -gt 0 ]]; do
  case "$1" in
    -y )
      FORCE_INSTALL=true
...
echo $FORCE_INSTALL
# Perform action based on the FORCE_INSTALL flag
if [ $FORCE_INSTALL = true ]; then
    echo "Force installation enabled."
    # Perform installation with force
else
    echo "Force installation not enabled."
    # Perform installation without force
fi
...
# If "-y" argument is not provided, ask for confirmation
...
echo
if [[ $FORCE_INSTALL = true || $REPLY =~ ^[Yy]$ ]]; then
